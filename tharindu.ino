...
void readAndDisplayTurbidity() {
  ...
  // Read and sum up 10 turbidity values
  for (int i = 0; i < 10; i++) {
  ...
  // Calculate average turbidity value
  int avgTurbidity = turbiditySum / 10;

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Turbidity: ");
  lcd.print(avgTurbidity);

  // Display turbidity status
  if (avgTurbidity <= 2000) {
    lcd.setCursor(0, 1);
    lcd.print("Good");
    strcpy(TurStatus, "Good");
  }else if(avgTurbidity <= 400){
    lcd.setCursor(0, 1);
    lcd.print("Average");
    strcpy(TurStatus, "Average");
  } else {
    lcd.setCursor(0, 1);
    lcd.print("Bad");
    strcpy(TurStatus, "Bad");
  }
}
